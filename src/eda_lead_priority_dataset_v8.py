...
REPORTS_DIR = Path("reports/eda_v8")
REPORTS_DIR.mkdir(parents=True, exist_ok=True)

INPUT_FILE = PROCESSED_DIR / "lead_priority_dataset_v8.csv"
TARGET_COL = "lead_is_converted"
SCORE_COL = "priority_score"
BUCKET_COL = "priority_bucket"
PERCENTILE_COL = "priority_percentile"
...
def score_review(df: pd.DataFrame) -> None:
    ...
    for col in [SCORE_COL, PERCENTILE_COL]:
        if col in df.columns:
            print(f"\n{col}:")
...
def bucket_review(df: pd.DataFrame) -> None:
    ...
    if BUCKET_COL in df.columns:
        ...
        if TARGET_COL in df.columns:
            ...
            bucket_target.to_csv(REPORTS_DIR / f"{BUCKET_COL}_target_rate.csv")
            ...
            plt.tight_layout()
            plt.savefig(REPORTS_DIR / f"{BUCKET_COL}_target_rate.png")
            plt.close()
...
def context_score_review(df: pd.DataFrame) -> None:
    print("\n" + "=" * 80)
    print("CONTEXT SCORE REVIEW")
    print("=" * 80)
