...
def build_lead_master_v3() -> pd.DataFrame:
    ...
    if "owner_hire_date" in lead_master.columns and "lead_created_date" in lead_master.columns:
        lead_master["owner_tenure_days"] = (
            lead_master["lead_created_date"] - lead_master["owner_hire_date"]
        ).dt.days
    ...
    if "last_task_created_date" in lead_master.columns and "lead_created_date" in lead_master.columns:
        lead_master["days_since_last_task_created"] = (
            lead_master["lead_created_date"] - lead_master["last_task_created_date"]
        ).dt.days
    ...
    if "last_task_activity_date" in lead_master.columns and "lead_created_date" in lead_master.columns:
        lead_master["days_since_last_task_activity"] = (
            lead_master["lead_created_date"] - lead_master["last_task_activity_date"]
        ).dt.days

    # Standardize booleans to numeric
    for col in ["lead_is_converted", "owner_is_active"]:
        if col in lead_master.columns:
    ...
    # Keep the master table clean and readable, but remove raw PII and unnecessary helper fields
    drop_cols = [
        "first_name",
        "last_name",
        "company",
...
def main() -> None:
    df = build_lead_master_v3()
    output_path = PROCESSED_DIR / "lead_master_v3.csv"
