...
REPORTS_DIR = Path("reports/modeling_v4")
...
DATASETS = {
    "with_owner": PROCESSED_DIR / "lead_model_ready_v4_with_owner.csv",
    "no_owner": PROCESSED_DIR / "lead_model_ready_v4_no_owner.csv",
}
...
def run_experiment(dataset_name: str, path: Path) -> pd.DataFrame:
    print("\n" + "=" * 80)
    ...
    comparison_file = REPORTS_DIR / f"model_comparison_{dataset_name}.csv"
    ...
    best_model_file = MODELS_DIR / f"best_model_{dataset_name}.joblib"
...
def main() -> None:
    ...
    for dataset_name, path in DATASETS.items():
        results_df = run_experiment(dataset_name, path)
        results_df["dataset"] = dataset_name
        summary_frames.append(results_df)

    summary = pd.concat(summary_frames, ignore_index=True)
    summary_file = REPORTS_DIR / "all_model_comparisons.csv"
    summary.to_csv(summary_file, index=False)

    print("\n" + "=" * 80)
    print("ALL DATASET RESULTS")
    print("=" * 80)
    print(summary)
